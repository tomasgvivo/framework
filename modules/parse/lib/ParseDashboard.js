...
const Authentication = require('parse-dashboard/Parse-Dashboard/Authentication.js');
var fs = require('fs');

__dirname = path.parse(require.resolve('parse-dashboard')).dir;

const currentVersionFeatures = require('parse-dashboard/package.json').parseDashboardFeatures;
...
module.exports = function(config, allowInsecureHTTP, app, mountPath) {
  app = app || express();
  // Serve public files.
  ...
  var mountHandler = function mountHandler(mountPath) {
  ...
  // If app is a router, it doesn't handle events.
  // So expect to receave a moutpath as a parameter.
  if(!app.on) {
    if(!mountPath) {
      throw new Error('A mountPath is expected when using Routers.');
    } else {
      mountHandler(mountPath);
    }
  } else {
    // wait for app to mount in order to get mountpath
    app.on('mount', function() {
      console.log('mountpath', getMount(app.mountpath));
    });
  }
