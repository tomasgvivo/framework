...
const rootPath            = global.rootPath || Path.dirname(require.main.filename);
...
class Framework {
  ...
  static require(moduleName, ...path) {
    ...
    Framework.logger.debug(`Module "${filename}" reqired.`);
  ...
  static include(moduleName, ...path) {
    let filename = Path.join(modules[moduleName], 'lib', ...path);
    Framework.logger.debug(`Module "${filename}" included.`);
    return require(filename);
  }
