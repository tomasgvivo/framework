...
class Framework {
   ...
   * @return {Module}
   */
  static _resourceResolver(moduleName, ...path) {
    let filename = Path.join(modules[moduleName], 'app', 'resources', ...path);
    return filename;
  }
  /**
   * Include a lib module file (js/json/node).
   ...
   * @return {Module}
   */
  static resource(moduleName, ...path) {
    let filename = Framework._resourceResolver(moduleName, ...path);
    let resource = resources[filename];
    Framework.logger.debug(`Resource "${filename}" required.`);
    if(resource && resource !== filename) {
      Framework.logger.debug(`Resource redirected to "${resource}".`);
      filename = resource;
    }
    let catched = resourcesCache[filename];
    if(catched) {
      return catched;
    }
   ...
   * @return {Module}
   */
  static resourceRewrite(fromResource, toResource) {
    fromResource = Framework._resourceResolver(...fromResource);
    toResource = Framework._resourceResolver(...toResource);
    resources[fromResource] === toResource;
  }
